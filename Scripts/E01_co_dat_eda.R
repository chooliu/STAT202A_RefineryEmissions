...
startdate_co <- as.Date("2021-08-01")
enddate_co <- as.Date("2024-11-01")


dat_co_2021 <-
  bind_rows(
  read_excel("Data/CCND/CCND_Q3_2021_Final_Data.xlsx", sheet = "CO"),
  read_excel("Data/CCND/CCND_Q4_2021_Final_Data.xlsx", sheet = "CO")) %>%
...
dat_co_2022 <-
  bind_rows(
    read_excel("Data/CCND/CCND_Q1_2022_Data.xlsx", sheet = "CO"),
    read_excel("Data/CCND/CCND_Q2_2022_Data.xlsx", sheet = "CO"),
    read_excel("Data/CCND/CCND_Q3_2022_Data.xlsx", sheet = "CO"),
    read_excel("Data/CCND/CCND_Q4_2022_Data.xlsx", sheet = "CO"),
  ) %>%
...
dat_co_2023A <-
  bind_rows(
    read_excel("Data/CCND/CCND_Q1_2023_Data.xlsx", sheet = "CO"),
    read_excel("Data/CCND/CCND_Q2_2023_Data.xlsx", sheet = "CO"),
    read_excel("Data/CCND/CCND Q4 2023 Data.xlsx", sheet = "CO"),
  ) %>%
  select(Date, contains("CM")) %>%
  mutate(Date = date(Date))
...
dat_co_2023B <-
  read_excel("Data/CCND/CCND Q3 2023 Data.xlsx", sheet = "CO") %>%
  select(Date, contains("CM")) %>%
  mutate(Date = date(Date))

names(dat_co_2023B)[-1] <-
  dat_co_2023B %>% names %>% .[-1] %>% str_extract("(CM[0-9]+)")

dat_co_2024 <-
  bind_rows(
    read_excel("Data/CCND/CCND Q1 2024 Data.xlsx", sheet = "CO", skip = 3)
  ) %>%
  select(Date, contains("CM")) %>%
  mutate(Date = date(Date))

names(dat_co_2024)[-1] <-
  dat_co_2024 %>% names %>% .[-1] %>% str_extract("(CM[0-9]+)")
...
dat_co <-
  ...
  select(Date, contains("CM")) %>%
  filter(year(Date) > 2000) # malformed strings resulting in dates in 1900
...
# start at 0
dat_co <- dat_co %>% mutate(DateInt = as.integer(Date))
offset_integer_co <- min(dat_co$DateInt, na.rm = T)

dat_co <-
  dat_co %>%
  mutate(DateIntNorm =
           DateInt - offset_integer_co)

dat_co_long <-
  dat_co %>%
  pivot_longer(cols = names(dat_co) %>% .[grepl("CM", .)],
...
quantile(dat_co_long$PolluteConc, na.rm = T)
grid_co <-
  seq(0, 3.5, length.out = 100)
...
dat_co_long %>%
  ...
  map(~ .x + scale_fill_viridis_c(label_co_short, option = "B") +
        geom_shadowtext(data = longlat_suncor, 
                        aes(Longitude, Latitude), label = "X", inherit.aes = F, size = 4) +
...
dat_co_long %>%
  ...
  geom_vline(xintercept = paste0(2021:2024, "-01-01") %>% as.Date(),
             lty = 1, alpha = 0.25) +
  geom_rug(data = tibble(emission = dates_co %>%
                           .[year(dates_co) > 2020] %>% as.Date),
           aes(x = emission),
           inherit.aes = F) +
  scale_x_date(breaks = paste0(2021:2024, "-01-01") %>% as.Date(),
               labels = (2021:2024)) +
  scale_y_continuous(label_co) +
