...
startdate_so2 <- as.Date("2021-08-01")
enddate_so2 <- as.Date("2024-11-01")


dat_so2_2021 <-
  bind_rows(
  read_excel("Data/CCND/CCND_Q3_2021_Final_Data.xlsx", sheet = "SO2"),
  read_excel("Data/CCND/CCND_Q4_2021_Final_Data.xlsx", sheet = "SO2")) %>%
...
dat_so2_2022 <-
  bind_rows(
    read_excel("Data/CCND/CCND_Q1_2022_Data.xlsx", sheet = "SO2"),
    read_excel("Data/CCND/CCND_Q2_2022_Data.xlsx", sheet = "SO2"),
    read_excel("Data/CCND/CCND_Q3_2022_Data.xlsx", sheet = "SO2"),
    read_excel("Data/CCND/CCND_Q4_2022_Data.xlsx", sheet = "SO2")
  ) %>%
...
dat_so2_2023A <-
  bind_rows(
    read_excel("Data/CCND/CCND_Q1_2023_Data.xlsx", sheet = "SO2"),
    read_excel("Data/CCND/CCND_Q2_2023_Data.xlsx", sheet = "SO2"),
  ) %>%
  select(Date, contains("CM")) %>%
  mutate(Date = date(Date))

# missing: "Data/CCND/CCND Q4 2023 Data.xlsx" and "Data/CCND/CCND Q3 2023 Data.xlsx"

# dat_so2_2024 <-
#   bind_rows(
#     read_excel("Data/CCND/CCND Q1 2024 Data.xlsx", sheet = "SO2", skip = 3)
#   ) %>%
#   select(Date, contains("CM")) %>%
#   mutate(Date = date(Date))
# 
# names(dat_so2_2024)[-1] <-
#   dat_so2_2024 %>% names %>% .[-1] %>% str_extract("(CM[0-9]+)")

# since late 2023 missing, maybe omit 2024 as well?
dat_so2 <-
  bind_rows(
    dat_so2_2021,
    dat_so2_2022,
...
# start at 0
dat_so2 <- dat_so2 %>% mutate(DateInt = as.integer(Date))
offset_integer_so2 <- min(dat_so2$DateInt, na.rm = T)

dat_so2 <-
  dat_so2 %>%
  mutate(DateIntNorm =
           DateInt - offset_integer_so2) %>%
  filter(!is.na(Date)) # seems to filter out two rows with non-zero values?

dat_so2_long <-
  dat_so2 %>%
  pivot_longer(cols = names(dat_so2) %>% .[grepl("CM", .)],
...
quantile(dat_so2_long$PolluteConc, na.rm = T)
...
compare_densities <-
  ...
  map_dfc(~ .x$PolluteConc %>%
            .[!is.na(.)] %>%
            wrap_gausskern(x = ., g = grid_so2))

# plot densities
compare_densities %>%
  set_names(., 1:ncol(.)) %>%
  bind_cols(conc = grid_so2, .) %>%
  pivot_longer(cols = 2:ncol(.)) %>%
  mutate(MonitorID = factor(name, levels = 1:10)) %>%
  ggplot(data = ., aes(conc, value, color = MonitorID)) +
  geom_line() +
  theme_few() +
  scale_x_continuous(label_so2, expand = c(0, 0), limits = c(0, max(grid_so2))) +
...
# 2, 8, 10? but all very close to 0
compare_densities %>% apply(., 2, which.max) %>% grid_so2[.]



# interpolated density at time t -----------------------------------------------

make_interpolation_plot_so2 <-
...
dat_so2_long %>%
  ...
  map(~ .x + scale_fill_viridis_c(label_so2_short, option = "C") +
        geom_shadowtext(data = longlat_suncor, 
                        aes(Longitude, Latitude), label = "X", inherit.aes = F, size = 4) +
...
dat_so2_long %>%
  ...
  geom_vline(xintercept = paste0(2021:2024, "-01-01") %>% as.Date(),
             lty = 1, alpha = 0.25) +
  geom_rug(data = tibble(emission = dates_so2 %>%
                           .[year(dates_so2) > 2020] %>% as.Date),
           aes(x = emission),
           inherit.aes = F) +
  scale_x_date(breaks = paste0(2021:2024, "-01-01") %>% as.Date(),
               labels = (2021:2024)) +
  scale_y_continuous(label_so2) +
